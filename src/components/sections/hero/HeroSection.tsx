...
/* ── Images ─────────────────────────────────────────────────────────────── */
const ALL_IMAGES = [
  "/assets/hero/3ac0d2c9715554614b1ec0e32e74cd606253df1a.png",
  "/assets/hero/45371db61b0ff5e4480bae84ce87842975dde27b.png",
  "/assets/hero/65f8215ff2ded03736c5f32f91751a0e5c94d6ce.png",
  "/assets/hero/7dd143dbcb005190af4aa40259cf6f77d99724cc.png",
  "/assets/hero/8e0a4aba5e948181f985a0c8f49cc83d02b7e6fc.png",
  "/assets/hero/aca36e01ed06f6af6cf93da04415684d26a65ce3.png",
  "/assets/hero/b08e85365c5657cce32392d152f1a516b95fd516.png",
  "/assets/hero/bc4ab278e0f3266e0e95d78e6b1c76733ca52ee3.png",
  "/assets/hero/fc89d9d6b47eebf9cf67d38fedb462629614f9c6.png",
];
...
/* ── ScrollColumn with momentum physics ─────────────────────────────────── */
interface ColProps {
  images: readonly string[];
  speed: number;         // px/frame — the "cruise" speed the column settles back to
  initOffset: number;
}
...
function ScrollColumn({ images, speed, initOffset }: ColProps) {
  ...
  const momentumRef = useRef(-speed);   // current velocity (negative = scrolling up)
  ...
  useEffect(() => {
    ...
    const tick = () => {
      if (!drag.current.active) {
        /*
         * Physics: target velocity is either the cruise speed (not paused)
         * or zero (paused/hovered). Momentum decays toward the target at
         * rate 0.04 per frame — like a tire settling.
         */
  ...
  const onPointerMove = (e: React.PointerEvent) => {
    ...
    // Keep a rolling 120ms window of velocity samples
  ...
  const stopDrag = () => {
    ...
    // Derive release velocity (px/frame at ~60 fps) from sampled history
    ...
    if (samples.length >= 2) {
      ...
      if (dt > 0) {
        // dy/dt in px/ms → px/frame (1000/60 ≈ 16.67ms per frame)
        const vel = ((last.y - first.y) / dt) * (1000 / 60);
        /*
         * vel > 0 means user dragged DOWN (position increases = reverse scroll).
         * vel < 0 means user dragged UP   (position decreases = faster scroll).
         * Clamp so a hard flick doesn't shoot the column out of view.
         */
  ...
  return (
    <section className="relative h-screen overflow-hidden bg-white flex items-center">
      ...
      <div className="relative z-10 flex flex-col justify-center px-8 md:px-16 xl:px-24 w-full lg:w-[52%] xl:w-[48%]">
        ...
        <p
          className="mt-6 text-[17px] leading-relaxed text-gray-500 max-w-[480px] hero-fade"
        ...
        <div
          className="mt-10 flex flex-wrap gap-3 hero-fade"
        ...
        {/* Stats */}
        <div
          className="mt-14 flex items-center gap-8 flex-wrap hero-fade"
