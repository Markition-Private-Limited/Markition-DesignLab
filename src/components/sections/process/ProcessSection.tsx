...
export function ProcessSection() {
  ...
  useEffect(() => {
    ...
    const onLenisScroll = (e: Event) => {
      /* Use Lenis targetScroll (unsmoothed) so steps respond instantly */
      const { targetScroll } = (e as CustomEvent<{ targetScroll: number }>).detail;
      const scrolledIn  = targetScroll - wrapperTopRef.current;
    ...
    document.addEventListener("lenis-target-scroll", onLenisScroll);
    window.addEventListener("resize", cacheTop, { passive: true });
    return () => {
      document.removeEventListener("lenis-target-scroll", onLenisScroll);
      ...
      >
        <div className="flex items-center justify-center h-full px-8 md:px-16 xl:px-24">
          <div className="flex items-start gap-12 w-full max-w-7xl">
            ...
            {/* LEFT — changes per step, key forces re-mount → animation resets */}
            <div className="w-[42%] flex-shrink-0">
              <div key={activeStep} className="step-left-in">
                <LeftPanel />
              </div>
            </div>

            {/* RIGHT — sliding steps list */}
            <div
